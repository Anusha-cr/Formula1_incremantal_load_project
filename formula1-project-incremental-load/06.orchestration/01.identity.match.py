...
if spark.catalog.tableExists(control_table):
    tarcked_batches =[
        ...
        for row in (
            ...
            .filter(F.col("status").isin("in_progres", "completed"))
            .select("batch_id")
            .distinct()
            .collect()
        )
...
new_batches = sorted(list(set(landing_batches) - set(tarcked_batches)))
next_batch = new_batches [0] if new_batches else None

print(f"landing_batches: {landing_batches}")
print(f"tarcked_batches: {tarcked_batches}")
print(f"new_batch to process: {next_batch}")
